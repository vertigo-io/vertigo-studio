...
public final class ShinyRatingRenderer implements ShinyComponentRenderer<ShinyRating> {
	...
	@Override
	public void render(final ShinyRating shinyRating, final ShinyWriter writer) {
		...
		if (shinyRating.scale() == ShinyRatingScale.SCALE_100 && maxValue == 100) {
			...
			buildProgressBarRating(shinyRating, rating, clampedValue, maxValue);
		} else {
			// For other scales, show individual icons
			buildIconRating(shinyRating, rating, clampedValue, maxValue);
		}
	...
	private static void buildIconRating(final ShinyRating shinyRating, final StringBuilder rating, final double clampedValue, final int maxValue) {
		for (int i = 1; i <= maxValue; i++) {
			...
			if (shinyRating.allowHalfRating() && clampedValue >= i - 0.5 && clampedValue < i) {
				...
				rating.append(ShinyEffects.DIM.apply(
						shinyRating.style().filledColor().fg(shinyRating.style().type().getFilledIcon())));
			} else if (clampedValue >= i) {
				// Filled
				rating.append(shinyRating.style().filledColor().fg(shinyRating.style().type().getFilledIcon()));
			} else {
				// Empty
				rating.append(shinyRating.style().emptyColor().fg(shinyRating.style().type().getEmptyIcon()));
			}
		}
	}

	private static void buildProgressBarRating(final ShinyRating shinyRating, final StringBuilder rating, final double clampedValue, final int maxValue) {
		...
		rating.append("[")
				.append(shinyRating.style().filledColor().fg(shinyRating.style().type().getFilledIcon().repeat(filledLength)))
				.append(shinyRating.style().emptyColor().fg(shinyRating.style().type().getEmptyIcon().repeat(barLength - filledLength)))
				.append("]");
	}
