    ...
    toHtml() {
        ...
        const itemsHtml = this.items.map(item => {
            ...
            if (item instanceof ChakraListComponent) {
                itemContent = `<span style="font-weight: bold; color: #90CDF4;">${item.title}</span>${item.toHtml()}`;
            } else {
                itemContent = item;
            }
            
            const liStyle = this.type === ChakraListComponent.TYPES.DASHED ? 'margin-bottom: 0.5em; color: #CBD5E0; position: relative; padding-left: 1em;' : 'margin-bottom: 0.5em; color: #CBD5E0;';
            const beforeStyle = this.type === ChakraListComponent.TYPES.DASHED ? 'content: "- "; position: absolute; left: 0;' : '';
