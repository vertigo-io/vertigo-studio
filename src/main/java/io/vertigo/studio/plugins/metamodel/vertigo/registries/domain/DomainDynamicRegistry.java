...
import io.vertigo.studio.metamodel.domain.DomainBuilder;
import io.vertigo.studio.metamodel.domain.FormatterDefinition;
import io.vertigo.studio.metamodel.domain.StudioDtDefinition;
import io.vertigo.studio.metamodel.domain.StudioDtDefinitionBuilder;
...
public final class DomainDynamicRegistry implements DynamicRegistry {
	...
	private StudioDtDefinition createStudioDtDefinition(final DefinitionSpace definitionSpace, final DslDefinition xdtDefinition) {
		...
		Assertion.check().argument(!dtDefinitionBuilders.containsKey(dtDefinitionName), "Definition '{0}' already registered", dtDefinitionName);
	...
	private static void populateIdDtField(
		...
		for (final DslDefinition field : fields) {
			...
			//--
			Assertion.check().argument(field.getPropertyNames().contains(KspProperty.LABEL), "Label est une propriété obligatoire");
	...
	private static void populateDataDtField(
		...
		for (final DslDefinition field : fields) {
			...
			Assertion.check().argument(field.getPropertyNames().contains(KspProperty.CARDINALITY), "cardinality is a required property.");
			//--
