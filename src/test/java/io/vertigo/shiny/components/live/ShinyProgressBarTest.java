...
public class ShinyProgressBarTest {
	...
	private static void testBasicProgressBar(final ShinyWriter writer) {
		...
		try (final ShinyProgressBar progressBar = Shiny.progressBar().total(100).start(writer)) {
			for (int i = 0; i < 100; i++) {
	...
	private static void testFastProgressBar(final ShinyWriter writer) {
		...
		try (final ShinyProgressBar progressBar = Shiny.progressBar().total(500).start(writer)) {
			for (int i = 0; i < 500; i++) {
				progressBar.liveUpdate(i + 1);
	...
	private static void testSlowProgressBar(final ShinyWriter writer) {
		...
		try (final ShinyProgressBar progressBar = Shiny.progressBar().total(50).start(writer)) {
			for (int i = 0; i < 50; i++) {
				progressBar.liveUpdate(i + 1);
